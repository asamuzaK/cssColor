...
const _SRGB = 'srgb(?:-linear)?';
export const ANGLE = 'deg|g?rad|turn';
export const CS_RGB = `(?:a98|prophoto)-rgb|display-p3|rec2020|${_SRGB}`;
export const CS_XYZ = 'xyz(?:-d(?:50|65))?';
export const CS_COLOR_MIX = `(?:ok)?l(?:ab|ch)|h(?:sl|wb)|${_SRGB}|${CS_XYZ}`;
...
export const SYN_COLOR_MIX_PART = `(?:${SYN_COLOR_TYPE})(?:\\s+${PCT})?`;
export const SYN_COLOR_MIX = `color-mix\\(\\s*in\\s+(?:${CS_COLOR_MIX})\\s*,\\s*${SYN_COLOR_MIX_PART}\\s*,\\s*${SYN_COLOR_MIX_PART}\\s*\\)`;
export const SYN_COLOR_MIX_CAPT = `color-mix\\(\\s*in\\s+(${CS_COLOR_MIX})\\s*,\\s*(${SYN_COLOR_MIX_PART})\\s*,\\s*(${SYN_COLOR_MIX_PART})\\s*\\)`;
