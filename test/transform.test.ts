...
import { ReadonlyColorMatrix } from '../src/typedef';
import * as transform from '../src/matrix/transform';
...
describe('validate color components', () => {
  const func = transform.validateColorComponents;
...
describe('normalize color components', () => {
  const func = transform.normalizeColorComponents;
...
describe('transform matrix', () => {
  const func = transform.transformMatrix;

  it('should transform vector correctly with identity matrix', () => {
    const identityMatrix: ReadonlyColorMatrix = [
      [1, 0, 0],
      [0, 1, 0],
      [0, 0, 1]
    ];
